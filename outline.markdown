...
* **Normalize rules:** `normalize_services`
...
* **Covert hosts to subnets:** `convert_hosts_in_rules`
...
* **Group rules for path detection:** `group_path_rules`
...
* **Identify subnet relations that must not be optimized at secondary routers:** `find_subnets_in_nat_domain`
...
* **Perform consistency checks:** `check_unused_groups`, `check_supernet_rules`, `check_expanded_rules` 
...
* **Remove duplicate path rules:** `remove_simple_duplicate_rules`
 :  To avoid multiple distribution of rules, duplicate rules are removed
    from the path rule set in this step.

* **Assure that rules allow access on every managed device:** `set_policy_distribution_ip`
...
* **Find routes for rules:** [`find_active_routes`](/Netspoc/technical.html#find_routes)
...
* **Identify rules that can be simplified at secondary routers:** `mark_secondary_rules`
...
* **Add raw files to program output:** `copy_raw`
...
* **Optimize router rule set** `optimize_rules`
...
* **Generate ACL configurations** `prepare_acls`
...
* **Write router configuration files:** `print_combined`
