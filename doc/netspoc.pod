...
recursivly from all files in input-dir and places one file for
each managed device into output-dir.
Alternativly input may be read from a single input-file; this
is mainly used for small examples or testing.

=head1 OPTIONS

Options may currently only be changed in the sourcefile of netspoc.
