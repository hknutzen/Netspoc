      ...
      if ($network->{nat}) {
         for my $nat (values %{ $network->{nat} }) {
            ...
            if (($nat->{ip} & $nat->{mask}) != $nat->{ip}) {
               error_atline "IP for $nat->{name} of doesn't", " match its mask";
   ...
   for my $area (values %areas) {
      if (my $name = $area->{anchor}) {
         if (my $network = $networks{$name}) {
            ...
            err_msg "Referencing undefined network:$name from $area->{name}";
               ...
               if ($ip % $modulo == 0) {
                  ...
                  if (my $neighbor = $ip2subnet->{$next_ip}) {
                     ...
                     if ($up_inv_prefix >= $network_inv_prefix) {
                        ...
                        and $up = $inv_prefix_aref[$up_inv_prefix]->{$ip})
                     {
                     }
                     else {
            ...
            for my $src (@src) {
               ...
               for my $dst (@dst) {
                  ...
                  for my $srv (@{ $unexpanded->{srv} }) {
                     ...
                     # If $srv is duplicate of an identical service, use the
                     # main service, but remember the original one for
                     # debugging / comments.
                     my $orig_srv;
		     ...
		     # Prevent modification of original array.
                     ...
                     if ($proto eq 'tcp' || $proto eq 'udp') {
                        ...
                        $srv       = $srv->{dst_range};
                        for my $src_range (expand_splitted_services $src_range)
                        {
                        ...
                        $src_range = $srv_ip;
                        $add_rule->(
                           $action, $src, $dst, $src_range, $srv, $unexpanded,
                           $orig_srv
                        );
                     }
                  }
               ...
               if ($mask_ip_hash{$m}->{$i}) {
                  ...
                  if ($strict_subnets) {
                        ...
                        and $subnet->{subnet_of} eq $bignet)
                     {
                        ...
                        # Prevent multiple error messages in different NAT
                        # domains.
...
sub setarea1( $$$ ) {
   ...
   for my $interface (@{ $any->{interfaces} }) {
      ...
      for my $out_interface (@{ $router->{interfaces} }) {
         ...
         if ($lookup->{$interface}) {
            err_msg "Invalid border $out_interface->{name} of $area->{name}";
            next;
         }
...
sub setany() {
   for my $any (@all_anys) {
      ...
      if (my $old_any = $network->{any}) {
         err_msg "More than one 'any' object defined in a security domain:\n",
...
#    };
   ...
   if ($from->{loop_exit} and my $loop_exit = $from->{loop_exit}->{$to}) {
      ...
      loop_path_walk $in, $loop_out, $from, $loop_exit, $at_router, $rule, $fun;
      unless ($loop_out) {
   ...
   if ($proto eq 'tcp' || $proto eq 'udp') {
      for my $range ($srv->{range}, $src_range->{range}) {
         (       $range->[0] == $range->[1]
           ...
           or err_msg "Crypto rule must not use $srv->{name} with port ranges";
      }
   ...
   for my $crypto (values %crypto) {
      ...
      if ($type eq 'ipsec') {
         my $ipsec = $ipsec{$name2}
           or err_msg "Can't resolve reference to '$type:$name2'", " for $name";
         $crypto->{type} = $ipsec;
         $use_ah = $ipsec->{ah};
...
sub check_any_src_rule( $$$ ) {
   ...
   unless (
      $rule_tree{ $rule->{action} }->{$out_any}->{$dst}->{ $rule->{src_range} }
      ->{ $rule->{srv} })
   {
...
sub check_and_convert_routes () {
   ...
   for my $router (@managed_routers) {
      ...
      for my $interface (@{ $router->{interfaces} }) {
         ...
         $interface->{hop} =
           [ sort { $a->{name} cmp $b->{name} } values %{ $interface->{hop} } ];
            ...
            for my $network (values %{ $interface->{routes}->{$hop} }) {
               if (my $interface2 = $net2intf{$network}) {
                  if ($interface2 ne $interface) {
                     ...
                     {
                        warn_msg "Two static routes for $network->{name}\n",
               ...
               unless ($interface->{routing}) {
                  if (my $hop2 = $net2hop{$network}) {
                     ...
                     if (   $hop->{virtual}
                        and $hop2->{virtual}
                        and $hop->{virtual}->{ip} eq $hop2->{virtual}->{ip})
                     {
            ...
            if (my $nat_network = $nat_map->{$network}) {
               if (my $nat_tag = $nat_network->{dynamic}) {
                  ...
                  unless ($obj->{nat}->{$nat_tag}) {
                     ...
                     # Permit whole network, because no static address is
                     # known.
                     $rule->{$where} = $network;
                  }
               }
   ...
   for my $this ('src', 'dst') {
      ...
      my $get_group = sub ( $ ) {
         ...
         for my $element (@{ $group->{elements} }) {
            is_any $element
              and internal_err "Unexpected $element->{name} in object-group";
         }
         ...
         for my $href (values %$href) {
            ...
            for my $href (values %$href) {
               ...
               for my $src_range_ref (keys %$href) {
                  ...
                  my @sorted =
                    sort { $a->{srv}->{range}->[0] <=> $b->{srv}->{range}->[0] }
                    (values %$href);
                  @sorted >= 2 or next;
                  my $i      = 0;
                  my $rule_a = $sorted[$i];
             ...
             RULE:
               for my $rule (@{ $hardware->{$rules} }) {
                                 ...
                                 if (my $hash = $hash->{$dst}) {
                                    while (1) {
                                       ...
                                       if (my $hash = $hash->{$src_range}) {
                                          while (1) {
                                             if (my $other_rule = $hash->{$srv})
                                             {
                                                unless ($rule eq $other_rule) {
                                                   $rule    = undef;
                                                   $changed = 1;
                                                   next RULE;
                                                }
                  ...
                  if ($secondary_router && $rule->{has_full_filter}) {
                     ...
                     # Add new rule to hash. If there are multiple rules which
                     # could be converted to the same secondary rule, only the
                     # first one will be generated.
...
sub print_crypto( $ ) {
   ...
   unless ($crypto_type) {
      err_msg "Crypto not supported for $router->{name} of type $model->{name}";
      return;
   }
   my $comment_char = $model->{comment_char};
   ...
   if (my $esp_ah = $ipsec->{esp_authentication}) {
      if ($esp_ah =~ /^(md5|sha)_hmac$/) {
         ...
         err_msg "Unsupported IPSec ESP auth. method for $crypto_type: $esp_ah";
      }
   }
