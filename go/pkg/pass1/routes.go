...
func (m netMap) addNetworks(list []someObj) {
LIST:
...
func generateRoutingTree1(rule *groupedRule, isIntf string, tree routingTree) {
	...
	pRule.srcNetworks.addNetworks(src)
	pRule.dstNetworks.addNetworks(dst)
		...
		for _, what := range strings.Split(isIntf, ",") {
			...
			if router.managed != "" || router.routingOnly {
				...
				if what == "src" {
					...
					for net, _ := range pRule.dstNetworks {
						m[net] = true
					}
				} else {
					...
					for net, _ := range pRule.srcNetworks {
						m[net] = true
					}
				}
			}
