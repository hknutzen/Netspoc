...
func clusterPathMark1(
	...
	if startInLoop != nil && inIntf.router == startInLoop.router {
		if inIntf != startInLoop {
			// Pathrestriction at startInLoop must be checked when
			// start router is left at other interface than startInLoop.
			if isBlocked(startInLoop) {
				return false
			}
		} else {
			// Ignore pathrestriction since path starts at border of zone.
		}
	} else {
	...
	// Also activate pathrestriction of startInLoop, if we are about
	// to leave the same router at some other interface.
	if startInLoop != nil && inIntf != startInLoop {
